...
class TableM extends React.Component {
  ...
  render() {
    ...
    const { count, page, rowsPerPage, dataChangedCounter } = this.state;
    const parseddata = this.parseData(data);
   const filteredrow =parseddata?.length;
    return (
      <>
        {pagination || paginationTop ? <PaginationComponent /> : null}
        ...
        >
          ...
          >
            <TableHead>
              <TableRow>
                {header?.map((Header, index) => (
                  ...
                  >
                    <Text variant="typo12">{Header.label}</Text>
                  </TableCell>
                ))}
              </TableRow>
              {/* </TableHead>
            ...
            > */}
              ...
              >
                {columns?.length > 0 &&
                  columns?.map((c, i) => {
                    return c.headerComponent || c.headerText ? (
                      <TableCell
                        key={`${c.field}_${i}`}
                        className={cx({
            ...
            >
              {parseddata?.map(({ rowData, isEditing }, i) => {
                return (
          ...
          {loading ? (
            <Box
              sx={{ width: "130px", position: "relative", minHeight: "300px" }}
            >
...
const SecondaryRow = (props) => {
  ...
  if (
    shouldRenderSecondaryRow &&
    shouldRenderSecondaryRow(rowData, rowIndex, colsData) === false
  ) {
    return null;
  }
...
const styles = (theme) => ({
  ...
  tableHeadCell: {
    ...
    fontSize: `${theme.typography.typo12.fontSize} !important`,
    border: "1px solid #EAECF0",
  },
  tableBody: {},
  tableBodyRow: {},
  tableBodyCell: {
    ...
    backgroundColor: "#fff",
    padding: "12px",
    fontSize: `${theme.typography.typo14.fontSize} !important`,
